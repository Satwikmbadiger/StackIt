...
export default function UserProfile() {
  ...
  return (
    <div style={{ background: "var(--theme-light-background)", color: "var(--theme-light-text-primary)", minHeight: "100vh", fontFamily: "var(--font-primary)", display: "flex", justifyContent: "center" }}>
      <div style={{ width: "70vw", maxWidth: 1200 }}>
        {/* Profile Header */}
        <section className="profile-hero" style={{ position: "relative", background: "var(--theme-light-surface)", minHeight: 220, display: "flex", alignItems: "flex-end", justifyContent: "center", overflow: "hidden", boxShadow: "var(--shadow-md)" }}>
          <div className="profile-glass" style={{ background: "var(--theme-light-surface)", borderRadius: "var(--radius-2xl)", boxShadow: "var(--shadow-md)", padding: "2rem 2.5rem", marginBottom: "-2rem", display: "flex", alignItems: "center", gap: "2.5rem", position: "relative", zIndex: 2 }}>
            <div className="profile-avatar" style={{ position: "relative", width: 120, height: 120, borderRadius: "50%", overflow: "hidden", boxShadow: "var(--shadow-md)", border: "3px solid var(--color-primary-500)", background: "#fff", flexShrink: 0 }}>
              <img src={currentUser.avatar || "https://randomuser.me/api/portraits/men/32.jpg"} alt="User Avatar" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
            </div>
            <div className="profile-info" style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.7rem" }}>
              <div className="profile-username" style={{ fontSize: "2rem", fontWeight: 700, color: "var(--theme-light-text-primary)", display: "flex", alignItems: "center", gap: "0.7rem" }}>
                {currentUser.username}
              </div>
              <div className="profile-displayname" style={{ fontSize: "1.2rem", color: "var(--theme-light-text-secondary)", fontWeight: 500 }}>
                {currentUser.displayName || currentUser.username}
              </div>
              <div className="profile-title" style={{ fontSize: "1.05rem", color: "var(--color-primary-500)", fontWeight: 500 }}>{currentUser.bio || ""}</div>
              <div className="profile-location" style={{ display: "flex", alignItems: "center", gap: "0.4rem", color: "var(--color-primary-500)", fontSize: "1rem" }}>{currentUser.location || ""}</div>
              <div className="profile-meta" style={{ display: "flex", gap: "1.2rem", fontSize: "1rem", color: "var(--theme-light-text-muted)", marginTop: "0.5rem" }}>
        ...
        {/* Quick Stats Bar */}
        <div className="profile-stats-bar" style={{ display: "flex", gap: "2rem", background: "var(--theme-light-surface)", borderRadius: "var(--radius-xl)", boxShadow: "var(--shadow-md)", padding: "1.2rem 2rem", margin: "2.5rem auto 0 auto", maxWidth: 900, justifyContent: "space-between", position: "relative", zIndex: 2 }}>
          <div className="stat-card" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.3rem", color: "var(--theme-light-text-primary)", fontSize: "1.1rem", fontWeight: 500, minWidth: 120, position: "relative", borderRadius: "var(--radius-lg)", background: "var(--theme-light-background)", boxShadow: "var(--shadow-sm)", padding: "1rem 0.7rem" }}>
        ...
        <div style={{ maxWidth: 900, margin: "2.5rem auto", display: "flex", gap: "2rem", flexWrap: "wrap" }}>
          ...
          <div style={{ background: "var(--theme-light-surface)", borderRadius: "var(--radius-xl)", boxShadow: "var(--shadow-md)", padding: "1.5rem", flex: 1, minWidth: 320 }}>
